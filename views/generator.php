				...
				<div class="card-body">	
					...
					<form id="generate-form" action="srv.php" method="POST">
						<div class="row">
							...
							<div class="col-md-4">
								<div class="form-group">
									<label for="crudName" class="form-label">CRUD Name: <span class="text-danger">*</span></label>
									<input type="text" name="crudName" id="crudName" class="form-control" placeholder="" maxlength="50" required>
								</div>
							</div>	
							<div class="col-md-4">
								<div class="form-group">
									<label for="crudTitle" class="form-label">CRUD Title: <span class="text-danger">*</span></label>
									<input type="text" name="crudTitle" id="crudTitle" class="form-control" placeholder="" maxlength="50" required>
								</div>
							</div>
						</div>	
						<div class="row">
...
$(document)
	...
	$("#table").change(function () {
		...
		if (table == ''){
			...
			$.ajax({
				url: "srv.php",
				data: {'act': 'getPrimaryColumnsByTable', 'database': database, 'table': table},
				type: "POST",
				beforeSend: function() {
				},				
				success: function (response) {
	...
	$("#table").change(function () {
		...
		if (table == ''){
			...
			$.ajax({
				url: "srv.php",
				data: {'act': 'getColumnsByTable', 'database': database, 'table': table},
				type: "POST",
				beforeSend: function() {
					$('#fields').html('');				
				},				
